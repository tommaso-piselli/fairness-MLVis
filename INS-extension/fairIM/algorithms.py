...
def lp_minmax(x, grad_oracle, k, group_indicator, group_targets):
    ...
    for i in range(len(group_targets)):
        if group_targets[i] > 0:
            m.addConstr(obj <= gp.quicksum(v[j]*g[j, i]
                        for j in range(len(v)))/group_targets[i])
...
#    startstep = stepsize
    ...
    for t in range(num_iter-1):
        ...
        iter_targets = group_targets - \
            val_oracle((1/num_iter)*x[0:t].sum(axis=0), 5000)
...
#        stepsize = startstep/np.sqrt(2*t+1)
        if solver == 'md':
            x[t+1] = mirror_sp((1./(t+1))*x[0:(t+1)].sum(axis=0), grad_oracle,
                               k, group_indicator, iter_targets, num_iter=1000)
        elif solver == 'gurobi':
            x[t+1] = lp_minmax((1./(t+1))*x[0:(t+1)].sum(axis=0),
                               grad_oracle, k, group_indicator, iter_targets)
        else:
...
def algo(grad_oracle, val_oracle, threshold, k, group_indicator, group_targets, num_iter, solver):
    ...
    print(f"Debug: Threshold Include Set S: {S}")
    grad_oracle = make_contracted_function(grad_oracle, S)
    val_oracle = make_contracted_function(val_oracle, S)
    x = multiobjective_fw(grad_oracle, val_oracle, k - len(S),
                          group_indicator, group_targets, num_iter, solver)
    x[:, list(S)] = 1
    return x
...
#        print(target)
        ...
        x = algo(grad_oracle, val_oracle, threshold, k,
                 group_indicator, group_targets, num_iter, solver)[1:]
